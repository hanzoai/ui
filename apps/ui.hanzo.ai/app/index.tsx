import { H1, Paragraph, Text, XStack, YStack } from '@hanzo/gui'
import { Head, Link, useLoader } from 'one'

export async function loader() {
  const { catalog } = await import('~/catalog')
  return { entries: catalog().map((e) => ({ name: e.name, title: e.title, members: e.members.length })) }
}

/** Every module @hanzo/ui exports, one card each, in the order the package names them. */
export default function Index() {
  const { entries } = useLoader(loader)
  return (
    <YStack maxW={1100} width="100%" self="center" px="$5" py="$8" gap="$6">
      <Head>
