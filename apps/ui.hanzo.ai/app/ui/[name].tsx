...
export async function generateStaticParams() {
  ...
  return catalog().map((e) => ({ name: e.name }))
}

export async function loader({ params }: LoaderProps<{ name: string }>) {
  const { catalog, examples, types } = await import('~/catalog')
  const { codeToHTML } = await import('@hanzogui/code-to-html')
  const all = catalog()
  const entry = all.find((e) => e.name === params.name)
  if (!entry) throw new Error(`no module named ${params.name}`)
  return {
    ...entry,
    nav: all.map((e) => ({ name: e.name, title: e.title })),
    types: types(entry.name).map((t) => codeToHTML(t, 'tsx')),
    examples: examples(entry.name).map((x) => ({ ...x, html: codeToHTML(x.source, 'tsx') })),
  }
}
...
/** One module of @hanzo/ui: its examples rendered, their source, and its types. */
export default function Page() {
  const doc = useLoader(loader)
  const mod = modules[`../../examples/${doc.name}.tsx`] ?? {}
  return (
    <XStack width="100%" maxW={1280} self="center">
      <Head>
        <title>{`${doc.title} — @hanzo/ui`}</title>
      </Head>
      <YStack
        width={220}
        py="$4"
      ...
      >
        {doc.nav.map((e) => (
          <Link key={e.name} href={`/ui/${e.name}`} style={{ textDecorationLine: 'none' }}>
            <Text
              fontSize={13}
              px="$2"
              py={4}
      ...
      <YStack flex={1} minW={0} px="$6" py="$6" gap="$6">
        <YStack gap="$2">
          ...
          <Paragraph fontFamily="$mono" fontSize={13} color="$color11">
            {`import { ${doc.members.filter((m) => !m.type).map((m) => m.name).join(', ')} } from '@hanzo/ui'`}
          </Paragraph>
        </YStack>
