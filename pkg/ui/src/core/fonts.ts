 ...
 * rather than the face. `theme.css` binds both to Zen; `@hanzo/font` declares
 * the @font-face and ships the binaries, and a surface imports it once at its
 * root. A host that wants its own face binds the two variables instead. The
 * components and `theme.css` read the variables, never a hard-coded font name,
 * and these constants are the one place the names are spelled so a host and the
 ...
 * No named system fonts here: `--font-sans` always resolves (theme.css binds it,
 * and a host may rebind it), so the generic is a last-resort generic rather than
 * a system-UI look.
 */
