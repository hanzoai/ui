...
import { DRAFT, REDACTED_MARKER } from './types'
 ...
 */
export function cardFields(dt: DocType, fields: FieldDefinition[], max = 4): FieldDefinition[] {
...
export function listedFieldNames(dt: DocType): string[] {
  return (dt.fields ?? []).filter((f) => f.inListView && !f.hidden).map((f) => f.fieldname)
}
...
export function mediaFileField(dt: DocType): string {
  const f =
    (dt.fields ?? []).find((x) => x.fieldtype === 'Attach' && x.reqd) ??
    (dt.fields ?? []).find((x) => x.fieldtype === 'Attach')
  return f ? f.fieldname : ''
}
 ...
 */
export function mediaDocPayload(dt: DocType, facts: MediaFacts): Record<string, unknown> {
 ...
 */
export function isDraft(record: Record<string, unknown> | null | undefined): boolean {
...
function coerceForWrite(f: DocField, v: unknown): unknown {
  switch (f.fieldtype) {
    ...
    case 'Link':
      ...
      if (v == null || v === '') return ''
      if (typeof v === 'object') {
        return String((v as { id?: unknown; value?: unknown }).id ?? (v as { value?: unknown }).value ?? '')
      }
      return v
    default:
      return v
