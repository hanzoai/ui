...
const mount = (node: React.ReactNode) => {
  ...
  return {
    ...
    screener: () => host.querySelector<HTMLElement>('[data-slot="forex-screener"]'),
    cleanup: () => {
      act(() => root.unmount())
      host.remove()
    },
...
describe('ForexScreener', () => {
  it('is a sized frame holding one TradingView iframe', () => {
    const markup = html(<ForexScreener />)
    const screener = tag(markup, 'forex-screener')
    ...
    expect(frame(markup)).toContain('title="forex screener"')
    // The iframe fills the frame; the frame is what carries the size.
    expect(frame(markup)).toMatch(/style="[^"]*width:100%;height:100%/)
  })

  it('defaults to the forex market on the overview column with the toolbar shown', () => {
    const result = settings(html(<ForexScreener />))
  ...
  it('resolves "forex" to forex_mkt, and passes any other market through as-is', () => {
    expect(settings(html(<ForexScreener market="forex" />)).screener_type).toBe('forex_mkt')
    expect(settings(html(<ForexScreener market="america" />)).screener_type).toBe('america')
    expect(settings(html(<ForexScreener market="crypto_mkt" />)).screener_type).toBe('crypto_mkt')
  })
  ...
  it('writes every prop under the name TradingView reads', () => {
    ...
    const screener = tag(markup, 'forex-screener')
    expect(screener).toContain('data-market="crypto_mkt"')
  ...
  it('keeps the table theme off the gui frame, and passes frame props through', () => {
    const markup = html(<ForexScreener colorTheme="light" rounded="$3" />)

    expect(markup.replace(/<style[\s\S]*?<\/style>/g, '')).not.toContain('t_light')
    expect(tag(markup, 'forex-screener')).not.toContain('data-theme')
    expect(tag(markup, 'forex-screener')).toContain('_btlr-c-radius-3')
  })

  it('mounts a new iframe when the market changes, and keeps it when nothing does', () => {
    const view = mount(<ForexScreener market="forex" />)
    const first = view.iframe()
    expect(first?.src).toContain(encodeURIComponent('"forex_mkt"'))

    view.render(<ForexScreener market="forex" />)
    expect(view.iframe()).toBe(first)

    view.render(<ForexScreener market="america" />)
